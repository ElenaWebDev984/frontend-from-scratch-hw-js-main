...
function updateImage() {
  imageElement.src = WEB_TECH_IMAGES[currentIndex]; // Update the src of the img tag
}

function showPrevImage() {
  currentIndex = (currentIndex - 1 + WEB_TECH_IMAGES.length) % WEB_TECH_IMAGES.length; // Calculate the previous index
  updateImage(); // Update the image displayed
}

function showNextImage() {
  currentIndex = (currentIndex + 1) % WEB_TECH_IMAGES.length; // Calculate the next index
  updateImage(); // Update the image displayed
}
